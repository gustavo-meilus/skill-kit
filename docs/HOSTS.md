...
Skill Kit is packaged for Claude Code, Codex, and Copilot CLI. The checked-in marketplaces currently list eight separately installable plugins. This table describes the package structure; a clean-install matrix is still required before making a launch claim for a specific host version.
